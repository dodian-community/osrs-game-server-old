...
import net.dodian.runescape.content.mechanics.NpcDeath;
...
import net.dodian.runescape.model.entity.npc.NpcMovementSync;
import net.dodian.runescape.model.entity.player.NpcSyncInfo;
import net.dodian.runescape.model.entity.player.PlayerDamageTracker;
...
public class Npc extends Entity {
	...
	public Npc(ScriptRepository scriptRepository, World world) {
		...
		hp = combatInfo == null ? 50 : combatInfo.stats.health;
	...
	private void fire_timers() {
		try {
			timerloop:
			for (Iterator<net.dodian.runescape.script.Timer> it = timers.timers().iterator(); it.hasNext(); ) {
				net.dodian.runescape.script.Timer entry = it.next();
	...
	@Override
	public int maxHp() {
		return combatInfo != null && combatInfo.stats != null ? combatInfo.stats.health : 50;
	}
	...
	public Optional<Integer> killer() {
		...
		if (account_id != -1) {
			...
			return Optional.of(account_id); // And find the highest damager :)
		} else {
