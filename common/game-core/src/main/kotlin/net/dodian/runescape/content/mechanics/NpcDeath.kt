...
import net.dodian.runescape.content.clearContext
import net.dodian.runescape.content.col
...
object NpcDeath {
	...
	val logger: Logger = LogManager.getLogger(NpcDeath.javaClass)
	...
	@JvmField val script: Function1<Script, Unit> = s@ {
		...
		val killerId: Optional<Int> = npc.killer()
		val optPlayer: Optional<Player> = if (killerId.isPresent) npc.world().playerForId(killerId.get()) else Optional.empty()
		// Player that did the most damage.
		val killer: Player? = if (killerId.isPresent && optPlayer.isPresent) optPlayer.get() else null
		...
		if (killer != null) {
			...
			val killerOpp: Entity? = killer.attribOr<Entity>(
				AttributeKey.LAST_DAMAGER, null);
		...
		it.delay(npc.combatInfo()?.deathLength ?: 5)
		...
		deathReset(npc)
		if (npc.respawns()) {
			InterruptibleTask.bound(npc)
				.execute(5) {
					npc.teleport(npc.spawnTile())
					npc.hidden(true)
				}
				.submit(TaskManager.npcEvents())

			val respawnTimeSeconds: Int = if (npc.combatInfo()?.respawnTime ?: 0 > 0) {
				logger.info("Using configured respawn time.")
				npc.combatInfo()?.respawnTime!!
			} else {
				logger.info("Using fallback respawn time.")
				30
			}

			val respawnTime: Int = secondsToTicks(respawnTimeSeconds)

			val deathTime = System.currentTimeMillis() / 1000;
			logger.info("NPC is now dead, and set to respawn in {} seconds.", respawnTimeSeconds)
			InterruptibleTask.bound(npc)
				.execute(respawnTime) {
					logger.info("NPC has now spawned after {} seconds.", (System.currentTimeMillis() / 1000) - deathTime)
					respawn(npc)
				}
				.submit(TaskManager.npcEvents())
		} else if (npc.id() != 6500) {
